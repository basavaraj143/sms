		...
		if (ctr != null) {
			if (ctr.moveToFirst()) {
				do {
					...
					//dateQuery = "DELETE FROM class_table c.class_id = '" + classId + "'; ";
...
*/
		...
		String teacher_id = tv1.getText().toString();

		try {
			//db.execSQL("insert into teacher_table VALUES ('basavaraj', 'basavaraj', 'basavaraj');");
